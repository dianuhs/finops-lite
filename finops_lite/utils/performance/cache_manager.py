...
class CacheManager:
    ...
    def _save_cache(self):
        ...
        try:
            ...
            cache_data = {key: entry.to_dict() for key, entry in self._cache.items()}
    ...
    def _manage_cache_size(self, cache_data: Dict):
        ...
        if cache_size_mb > self.max_cache_size_mb:
            ...
            sorted_entries = sorted(cache_data.items(), key=lambda x: x[1]["timestamp"])
    ...
    def get(
        self, operation: str, data_type: str = "cost_data", **kwargs
    ) -> Optional[Any]:
        """
        Get cached data.
