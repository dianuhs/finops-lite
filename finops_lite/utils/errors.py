            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if isinstance(e, APIRateLimitError):
                        console.print(f"[yellow]Rate limited, retrying in {delay:.1f}s... (attempt {attempt + 1}/{max_retries})[/yellow]")
                    elif isinstance(e, NetworkTimeoutError):
                        console.print(f"[yellow]Network timeout, retrying in {delay:.1f}s... (attempt {attempt + 1}/{max_retries})[/yellow]")
                    else:
                        console.print(f"[yellow]Temporary error, retrying in {delay:.1f}s... (attempt {attempt + 1}/{max_retries})[/yellow]")
        ...
        try:
            ...
            if 'credentials' in error_message or 'access denied' in error_message:
                if 'cost explorer' in error_message:
                    raise CostExplorerNotEnabledError("Cost Explorer not enabled or insufficient permissions")
                raise AWSCredentialsError(f"AWS credentials error: {e}")
            
            elif 'cost explorer' in error_message:
                if 'data is not available' in error_message or 'warming up' in error_message:
                    raise CostExplorerWarmingUpError("Cost Explorer data not yet available (still warming up)")
                elif 'not enabled' in error_message:
                    raise CostExplorerNotEnabledError("Cost Explorer not enabled in this account")
                else:
...
def validate_aws_region(region: Optional[str]) -> Optional[str]:
    ...
    if region not in valid_regions:
        console.print(f"[yellow]⚠️  Region '{region}' not in common regions list[/yellow]")
